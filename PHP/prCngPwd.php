...
<body>
  ...
  if(isset($_POST['button']))
  {
    ...
    {
      $sql = "SELECT password FROM principal WHERE pid='".$_SESSION["pid"]."'";
    }
    ...
    if(mysqli_num_rows($res) > 0)
    {
      if($cpwd == $row["password"])
      {
        ...
        if($npwd == $rnpwd)
        {
          $sql =  "UPDATE principal SET password='".$npwd."' WHERE password='".$cpwd."'";
          $res = mysqli_query($conn,$sql);
        ...
        {
          echo "<script>alert('Password Not Match'); window.location.href='../PHP/prCngPwd.php';</script>";
        }
      }
    }
